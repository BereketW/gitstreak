...
export default function PullRequestsScreen() {
    ...
    const renderSkeleton = () => (
        <View className="px-5 pb-32 space-y-5">
    ...
    const renderItem = ({ item: pr }: { item: GithubPR }) => {
        const repoName = getRepoName(pr.repository_url);
        
        return (
            <TouchableOpacity className={`bg-white dark:bg-[#111827] border ${pr.draft ? 'border-slate-200 border-dashed dark:border-white/10 opacity-90' : 'border-slate-200 dark:border-white/5'} rounded-3xl p-5 shadow-sm active:scale-[0.98] transition-transform mb-5 mx-5`}>
                <View className="flex-row items-start gap-3 mb-3">
                    <View className="mt-1">
